...
describe("todos actions creators", () => {
   it("should handle success action", async () => {
    ...
    it("should handle error action", async () => {
      mockAxios.get.mockImplementationOnce(() => 
      Promise.reject({ response: {data: { error: "algo salió mal" } }})
      );
      const store = mockStore();
      await store.dispatch(getTodos());
      const actions = store.getActions();
...
describe("todos reducer", () => {
  ...
  it("should handle getTodoError", () => {
    const action = getTodosError({ error: "algo salió mal" });
    const state = reducer(undefined, action);
    expect(state).toStrictEqual(buildState({ error: "algo salió mal" }));
  });
});